...
# Usage : python -m rupture.pca [media]
import os
import sys
import time
...
if __name__ == "__main__":
    ...
    debut = time.time()
    F0 = np.load(f"{DOSSIER}/fenetres_{media}.npz")["fenetres"]
    print(f"{media} : {F0.shape[0]} fenetres x {F0.shape[1]} jours", flush=True)
    ...
    for norme in ("z", "01", "col"):
        ...
        np.savez_compressed(
            f"{DOSSIER}/pca_{media}_{norme}.npz",
    ...
    fig.tight_layout()
    fig.savefig(f"{SORTIES}/pca_{media}_variance.png", bbox_inches="tight", dpi=200)
    plt.close(fig)
    ...
    fig.savefig(f"{SORTIES}/pca_{media}_composantes.png", bbox_inches="tight", dpi=200)
    plt.close(fig)

    print(f"FINI en {time.time() - debut:.0f} s -> pca_{media}_<norme>.npz, "
          f"{os.path.relpath(SORTIES)}/pca_{media}_*.png", flush=True)
