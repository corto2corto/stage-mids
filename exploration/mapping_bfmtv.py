"""Construit la liste des URLs de contenus BFMTV via l'index
sitemap_index_arbo_contenu.xml : sitemaps mensuels gzippes
sitemaps/rubriquesContenus/YYYY-MM-N.xml.gz (couvre 2006 -> aujourd'hui,
~460 fichiers ; les deux sitemaps de rubriques sont exclus par le motif).
Structure reperee lors de la reco -- cf discussion.

    python -m exploration.mapping_bfmtv
...
SORTIE = "exploration/bfmtv_url.csv"
ENTETES = {"User-Agent": "Mozilla/5.0 (recherche academique, mapping-agent)"}
MOTIF_SOUS_SITEMAP = re.compile(r"<loc>(https://www\.bfmtv\.com/sitemaps/rubriquesContenus/\d{4}-\d{2}-\d\.xml\.gz)</loc>")
