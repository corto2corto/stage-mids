...
\[UPDATE\] Après passage par Chrome, il est possible d'accéder aux articles payants des Échos.
...
\[UPDATE\] Réussi avec l'extension Chrome. \[RE-UPDATE\] N'a pas l'air de fonctionner complètement. Fonctionne avec l'extension Chrome. A appartenu à Drahi (2006-2015), puis propriété d'Alain Weill (2019-2023).
...
\[UPDATE\] Accessible ("Valeurs Actuelles Club") via le bypass Chrome.
...
Pas d'articles payants (à première vue), et des archives propres disponibles de 2015 à 2026.

### Télérama 

Fonctionne avec Chrome. Rachat par Le Monde, donc influencé par le changement d'actionnaires principaux.
...
Une première approche est de chercher via un script `sitemap_finder.py` un fichier sitemap.xml (ou équivalent), car c'est le fichier utilisé par Google pour indexer le site. En général, le sitemap est indiqué dans le fichier `robots.txt`.
...
Le problème est que, comme pour le site web, aucun article avant 2022 ne semble disponible.
...
Sitemap tentaculaire, pas très clair.
...
Premières recherches via Internet Archive. Tester la Wayback Machine aussi.
Ici, on a les archives https://www.parismatch.com/sitemap.xml de 2022 à 2026.
L'idée est d'utiliser la Wayback Machine afin de récupérer d'anciens sitemaps : http://web.archive.org/cdx/search/cdx?url=parismatch.com/sitemap*&output=json&collapse=urlkey
...
Assez complexe mais satisfaisant : on peut accéder à 3 gros sitemaps (trop gros pour charger normalement) via https://www.valeursactuelles.com/sitemap_index.xml.
Le site de VA est un peu particulier, car il est divisé en 4 catégories (VA, VA Club, VA Régions, VA+). Reste à voir si chaque catégorie est accessible via le sitemap. Dans tous les cas, il contient beaucoup de liens.
...
Les premières années sont peu denses, mais à partir de 2006, énormément d'articles disponibles.
