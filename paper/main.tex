...
The goal of this section is to find some regular time patterns around activity spikes. For this, we perform a PCA on time windows around spikes. More precisely, let $w$ be a word for which there is an anomalous activity spike at a certain day $t$. Let $D$ be a window size. We exctract a window of $D$ time units before and after the spike at time $t$, obtaining a vector of size $2D+1$, $Z^w_t = (X_{t-D}, \dots, X_{t-1}, X_t, X_{t+1}, \dots, X_{t+D})$. If $n$ is the total number of anomalous activity spikes detected in the corpus, we thus obtain a matrix $Z = (Z^w_t)_{w \in \mathcal{W}, t = 1, \dots, n}$ of size $n \times (2D+1)$. 
