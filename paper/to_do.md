...
- [ ] Extraire les séries du vocabulaire retenu : script à part optimisé pour la performance (une seule passe sur `unigram`), pas une boucle sur `serie()` ; fusion des graphies **exacte** (somme jour par jour, pas le max du recensement).
