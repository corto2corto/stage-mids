...
- `campagne_pca/suivi.md` — journal horodaté + prompt de reprise (source de
  vérité pour reprendre la campagne dans une nouvelle session).
- `campagne_pca/resultats.csv` — copie Mac (scp depuis le serveur) du
  `data/campagne/resultats.csv`, committée régulièrement.
- `campagne_pca/vocab_categories.csv` — mot → catégorie grammaticale
