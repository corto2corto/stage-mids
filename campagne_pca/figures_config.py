# Figures d'une configuration PCA quelconque (memes vues que figures_optimale.py :
# spectre, profils des composantes, plan PC1-PC2, archetypes, reconstruction),
# parametrable en ligne de commande pour comparer plusieurs configurations.
# Usage : .venv/bin/python -m campagne_pca.figures_config <media> --demi D --seuil S --prefixe nom
import argparse
import os
...
print(f"{MEDIA} d{DEMI} s{SEUIL:g} : {len(Z)} fenetres x {D} blocs | "
      f"K50={K50} (K50_frac={K50 / rang:.2f}), cum6={cum[min(5, rang - 1)] * 100:.1f} %")
...
ax.set_title(f"Variance expliquée par composante — {K50} composantes pour 50 % "
             f"(K50/rang={K50 / rang:.2f})\n{TITRE}", fontsize=9.5, color=ENCRE2)
fig.tight_layout()
fig.savefig(f"{FIGURES}/{PREFIXE}_spectre.png", bbox_inches="tight", dpi=200)
plt.close(fig)
...
fig.savefig(f"{FIGURES}/{PREFIXE}_composantes.png", bbox_inches="tight", dpi=200)
plt.close(fig)
...
for x, y, texte, mot in places:
    ax.annotate(texte, (x, y), xytext=(9, 4), ha="left",
...
fig.tight_layout()
fig.savefig(f"{FIGURES}/{PREFIXE}_plan12.png", bbox_inches="tight", dpi=200)
plt.close(fig)
...
fig.savefig(f"{FIGURES}/{PREFIXE}_reconstruction.png", bbox_inches="tight", dpi=200)
plt.close(fig)

print(f"-> {os.path.relpath(FIGURES)} : {PREFIXE}_spectre.png, {PREFIXE}_composantes.png, "
      f"{PREFIXE}_plan12.png, {PREFIXE}_archetypes.png, {PREFIXE}_reconstruction.png")
