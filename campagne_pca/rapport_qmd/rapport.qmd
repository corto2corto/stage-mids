...
```{python}
# Figures calculees ici : synthese_* depuis data/resultats_rotation.csv (calcul
# leger), optimale_* depuis le cache PCA (data/cache_pca/). Aucun png sur le
# disque. Cache construit sur le serveur par scripts/construire_cache.py.
import sys
...
d, pres, seuil_vol, eligibles = lib.config("optimale")
```
...
1. **La dimension.** Une fenêtre ±5 vit dans 10 dimensions, une ±50 dans
   ...
   hypothèse nulle : plus il est élevé, plus la variance réellement
   expliquée dépasse celle qu'on attendrait sans aucune structure.
...
```{python}
#| label: fig-synthese-axes
#| fig-cap: "Effets des axes sur les deux métriques (brut, n apparié, grille équilibrée sur 39 cellules × 4 médias)."
r_synthese = lib.donnees_synthese()
lib.vue_synthese_axes(r_synthese, None)
```
...
```{python}
#| label: fig-synthese-carte
#| fig-cap: "Chaque point est un média × grille à la configuration de référence. La diagonale négative résume la campagne : la part ancrée s'échange contre la texture."
lib.vue_synthese_carte(r_synthese, None)
```
...
(`campagne_pca/scripts/figures.py`, commande `config`).

```{python}
#| label: fig-optimale-spectre
#| fig-cap: "Variance expliquée par composante. La ligne rouge est le repère d'un nuage sans structure (100/20 = 5 %) : cinq composantes la dépassent."
...
```{python}
#| label: fig-optimale-composantes
...
lib.vue_plan12(d, pres, None, pres.decalages)
```

```{python}
#| label: fig-optimale-archetypes
...
```{python}
#| label: fig-optimale-reconstruction
#| fig-cap: "Reconstruction d'une fenêtre par les premières composantes."
...
- **Boilerplate du Figaro** (vérifié le 14/08/2026 sur la base ngram et le
  corpus) : deux blocs de texte de gabarit sont comptés comme des mots. La
  ligne de crédit `Source : AFP` des brèves « Flash Actu » multiplie
  « source » par 25 de novembre 2007 à mai 2008 — 562 articles sur 1 500 en
  février 2008, à volume de corpus constant ; l'encart publicitaire
  `SERVICE >> 100 € offerts par bwin.fr` triple « service » de septembre à
  décembre 2011. Ce sont des plateaux temporaires, pas des marches : la
  fenêtre de ±15 jours n'en voit qu'un bord. Détecteur générique trouvé au
  passage : le taux de continuation en bigramme s'effondre sur un mot de
  gabarit (76 % hors plateau, 8 % pendant, la tokenisation coupant sur les
  « : »). Nettoyage à faire avant toute interprétation fine des composantes
  du Figaro (chantier commun avec les stop words).
- **Mediapart, en revanche, n'a pas de boilerplate** : « articles »
  (27 occurrences le jour le plus fourni) et « phrases » (11) sont des mots
  ordinaires à faible effectif sur un corpus dix fois plus petit que celui
  du Figaro (~11 000 mots/jour). S'ils remontaient dans les archétypes,
  c'est parce que le z-score efface l'échelle — le filtre de volume les
  écarte, et les exclure ne change presque rien à la métrique
  (2,55 → 2,48).
...
- **Kernel PCA** : explorée depuis sur les quatre médias et trois grilles
  (`kernel_pca.qmd`), conclusion négative — le noyau RBF n'a jamais concentré
  la variance mieux que la PCA linéaire.
...
(`rupture/campagne.py`), récoltes (`data/resultats.csv`, 1 753 configs ;
`data/resultats_rotation.csv`, 2 304 configs au double témoin), cache PCA
(`data/cache_pca/`) et scripts de figures (`scripts/figures_lib.py`,
`scripts/figures.py`). Les données intermédiaires (pics s3, séries 40k, base
ngram Mediapart) sont sur gallica dans `data/`. Le catalogue des
configurations candidates (`configurations.qmd`) et l'exploration Kernel PCA
(`kernel_pca.qmd`) complètent ce rapport.
