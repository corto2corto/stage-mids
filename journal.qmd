...
python -m scripts.suivi tendance           # page HTML du taux de réussite
```
...
```bash
cd /data/elias/stage-mids && source .venv/bin/activate
python -m scripts.suivi tendance
...
`pipeline.py` charge les URLs au démarrage ; `creer_csv.py` crée le fichier de sortie. Pour trouver le bon sélecteur CSS, utiliser `exploration/recuperer_html.py`.
