...
- **Rattrapage avant versement** — les mappings sont désormais relançables :
  [`mapping_cnews.py`](exploration/mapping_cnews.py) passe en append+dédup
  (re-balayage obligatoire, sitemap non trié par date) et
  [`mapping_20minutes.py`](exploration/mapping_20minutes.py) prend les années
...
- 20minutes rejoint la collecte news quotidienne
  ([`sitemap_news.py`](exploration/sitemap_news.py)) ; cnews n'a pas de
  sitemap news exploitable → re-balayage périodique de son sitemap à prévoir
  (~40 min, le mapping se relance tel quel).
