...
On a maintenant la liste des URLs (≈ 10 millions). Reste à aller chercher le contenu de chaque article. L'idée : garder toutes les URLs dans une petite base, marquées « à faire », et les traiter par **batchs** jusqu'à épuisement. À chaque batch, une URL par journal, scrapée en parallèle.
...
| Module | Rôle |
|---|---|
| [`batch.py`](scraping/batch.py) | choisit les URLs du prochain batch |
| [`navigateur.py`](scraping/navigateur.py) | pilote Firefox + bypass, récupère le HTML |
| [`extraction.py`](scraping/extraction.py) | HTML → titre, auteur, date, section, corps |
| [`paywall.py`](scraping/paywall.py) | vérifie que l'article n'est pas tronqué |
| [`stockage.py`](scraping/stockage.py) | écrit le CSV, met à jour l'état en base |
| [`pipeline.py`](scraping/pipeline.py) | enchaîne le tout, batch après batch |

### La base `urls.db` : une todo-list

Une seule table SQLite, quatre colonnes : `id`, `media`, `url`, `etat`. Tout est piloté par `etat` :
...
| `3` | déjà disponible ailleurs, inutile de scraper |

La base ne stocke pas les articles, juste leur état d'avancement (le texte part dans des CSV, un par journal). Elle ne répond qu'à « qu'est-ce qu'il reste à faire ? », et c'est ce qui rend le scraping reprenable : les URLs encore à `0` sont reprises au prochain lancement.

### Préparation : écarter les doublons

Avant de scraper, on retire ce qui existe déjà ailleurs. Une bonne partie de nos URLs sont déjà publiées, métadonnées comprises, dans un jeu de données public : [`regicid/press_metadata`](https://huggingface.co/datasets/regicid/press_metadata) (~11,17 M d'URLs de presse française). [`scripts/4_marquer_doublons.py`](scripts/4_marquer_doublons.py) les passe à l'`etat = 3`, et le scraping (qui ne traite que les `etat = 0`) les ignore. Résultat : **7 273 332 doublons écartés**.

### `batch.py` : constituer un batch

[`batch.py`](scraping/batch.py) renvoie une URL par journal encore à traiter :

```sql
SELECT media, id, url FROM urls WHERE etat=0 GROUP BY media
```

On obtient un dictionnaire `{media: (id, url)}`. Pourquoi une seule par journal ? Parce qu'on ouvre **un navigateur par journal** tournant en parallèle : une URL par navigateur à chaque tour répartit la charge et fait avancer tous les journaux en même temps.

### `navigateur.py` : piloter Firefox avec le bypass

C'est ici qu'on récupère le HTML. La difficulté : la plupart des articles sont derrière un paywall.

On utilise **Firefox plutôt que Chrome** : son API Selenium expose `install_addon`, qui installe une extension au démarrage, y compris non signée (bypass-paywalls-clean + uBlock Origin). Pas de profil pré-configuré sur le serveur : on part d'un Firefox vierge et on injecte les `.xpi` à chaud.

Trois fonctions :

- `configurer_ublock()` — appelée **une fois**. uBlock ne marche pas tel quel : il faut activer les bonnes listes de filtres (dont les anti-bandeaux de cookies, qui sinon bloquent le bypass), passées via un fichier JSON lu au démarrage (*managed storage*).
...
- `scraper(driver, url)` — vide les cookies (certains journaux ne déclenchent le paywall qu'au-delà d'un quota d'articles compté en cookie), charge la page, attend ~8 s que le bypass agisse, lit le HTML.

Un navigateur est ouvert une fois par journal puis réutilisé : démarrer Firefox coûte cher. Côté serveur : `--headless`, images désactivées (inutiles pour du texte), et un Firefox + geckodriver « maison » plutôt que la version *snap* d'Ubuntu, qui plante avec nos profils sur `/data`.

### `extraction.py` : du HTML aux métadonnées et au corps

On a le HTML brut, il faut en sortir le **titre, l'auteur, la date, la rubrique, l'accès libre/payant** et le **corps**. Problème : chaque journal range ça différemment.

**Métadonnées → JSON-LD.** La plupart des sites insèrent un bloc de données structurées [schema.org](https://schema.org) (`<script type="application/ld+json">`), où les champs portent les mêmes noms partout (`headline`, `datePublished`, `author`, `articleSection`…). Au lieu de chercher des balises différentes par journal, on lit les mêmes champs partout. Présent sur 9 médias sur 10, souvent plus complet que les `<meta>`.
...
**Un dictionnaire route chaque média**, en indiquant *où* lire les métadonnées et *où* trouver le corps. Par exemple :
...
### `paywall.py` : vérifier que le bypass a réussi

Le bypass ne marche pas à tous les coups. [`est_bloque(contenu)`](scraping/paywall.py) renvoie `True` si le contenu est vide, ou si une phrase-signal de troncature apparaît dans les **300 derniers caractères** (« il vous reste X % à découvrir », « réservé aux abonnés »…) — c'est là que ces messages se logent. Un article bloqué reçoit `etat=1` et n'est pas écrit.
...
### `stockage.py` : écrire les résultats
...
- `maj_bdd(...)` met à jour l'`etat` de l'URL. Le `commit` est laissé à l'appelant, pour grouper tout un batch en une transaction.

### `pipeline.py` : orchestrer les batchs
...
[`scripts/suivi.py`](scripts/suivi.py) lit la base et les CSV (en lecture seule) et expose des indicateurs en ligne de commande :

```bash
python -m scripts.suivi                    # vue d'ensemble
python -m scripts.suivi avancement         # URLs traitées par média
python -m scripts.suivi contenu le_monde   # longueur des articles
```

**Détection des décrochages.** Sur un run de plusieurs jours, le HTML d'un journal peut changer et casser son bypass sans qu'on le voie. Le pipeline appelle donc `snapshot()` tous les 1000 articles et surveille le taux de réussite récent avec une **carte de contrôle** : si un journal passe sous *moyenne − 3σ* de son historique, une notification part via [ntfy](https://ntfy.sh).
